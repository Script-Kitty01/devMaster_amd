...
msg = json.dumps({
    'header': {'msg_id': 'qc1', 'username': 'u', 'session': 's', 'msg_type': 'execute_request', 'version': '5.3'},
...
while time.time() < timeout:
    ...
    try:
        ...
        if data.get('msg_type') == 'stream' and data.get('parent_header', {}).get('msg_id') == 'qc1':
            print(data.get('content', {}).get('text', ''), end='')
        elif data.get('msg_type') == 'execute_result' and data.get('parent_header', {}).get('msg_id') == 'qc1':
            print(data.get('content', {}).get('data', {}).get('text/plain', ''))
        elif data.get('msg_type') == 'error' and data.get('parent_header', {}).get('msg_id') == 'qc1':
            print('ERROR:', data.get('content', {}).get('ename', ''), data.get('content', {}).get('evalue', ''))
        elif data.get('msg_type') == 'status' and data.get('content', {}).get('execution_state') == 'idle' and data.get('parent_header', {}).get('msg_id') == 'qc1':
            break
    except:
        break
ws.close()
