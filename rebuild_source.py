...
# Force source build with --no-binary
env = os.environ.copy()
env["CMAKE_ARGS"] = "-DGGML_HIPBLAS=ON -DAMDGPU_TARGETS=gfx1100"
env["FORCE_CMAKE"] = "1"

print("=== Force source build with --no-binary ===")
cmd = [
...
for line in (result.stdout + result.stderr).split(chr(10)):
    low = line.lower()
    if any(kw in low for kw in ["hip", "rocm", "gpu", "blas", "error", "fail", "ggml_hip", "amd", "gfx", "building", "cmake", "success"]):
        print(line[:300])
