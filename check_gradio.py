...
pid = r.stdout.strip()
if pid:
    r = subprocess.run(['ps', '-p', pid, '-o', 'pid,pcpu,pmem,etime,args'], capture_output=True, text=True)
    print('Gradio process:')
    print(r.stdout)

# Check GPU
r = subprocess.run(['/opt/rocm/bin/rocm-smi', '--showuse'], capture_output=True, text=True)
print('\nGPU Usage:')
print(r.stdout[:500])
"""
