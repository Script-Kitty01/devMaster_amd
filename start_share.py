...
# Kill ALL gradio processes forcefully
os.system('pkill -9 -f gradio_app 2>/dev/null')
time.sleep(3)
...
s.close()
print('Port 7860 before:', 'FREE' if r != 0 else 'IN USE')

# Create a standalone script with share=True
script = '''
import sys
...
with open('/tmp/gradio_share.py', 'w') as f:
    f.write(script)

# Start it
proc = subprocess.Popen(
    ['/opt/venv/bin/python', '/tmp/gradio_share.py'],
    stdout=open('/tmp/gradio_share_out.txt', 'w'),
    stderr=open('/tmp/gradio_share_err.txt', 'w'),
    start_new_session=True
)
print(f'Started PID: {proc.pid}')
print('Waiting 60s for model load + share tunnel...')
time.sleep(60)
...
r = s.connect_ex(('127.0.0.1', 7860))
s.close()
print('Port 7860 after:', 'OPEN' if r == 0 else 'CLOSED')
"""
