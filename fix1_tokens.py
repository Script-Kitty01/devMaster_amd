...
# FIX 1: Increase max_tokens in rocm_service.py
with open('src/llm/rocm_service.py', 'r') as f:
    content = f.read()

count = content.count('max_tokens=256')
...
with open('src/llm/rocm_service.py', 'w') as f:
    f.write(content)
print(f'FIX 1: Replaced {count} occurrences of max_tokens=256 in rocm_service.py')
"""

msg = json.dumps({
    'header': {'msg_id': 'f1', 'username': 'u', 'session': 's', 'msg_type': 'execute_request', 'version': '5.3'},
...
while time.time() < timeout:
    ...
    try:
        ...
        if data.get('msg_type') == 'stream' and data.get('parent_header', {}).get('msg_id') == 'f1':
            print(data.get('content', {}).get('text', ''), end='')
        elif data.get('msg_type') == 'execute_result' and data.get('parent_header', {}).get('msg_id') == 'f1':
            print(data.get('content', {}).get('data', {}).get('text/plain', ''))
        elif data.get('msg_type') == 'error' and data.get('parent_header', {}).get('msg_id') == 'f1':
            print('ERROR:', data.get('content', {}).get('ename', ''), data.get('content', {}).get('evalue', ''))
        elif data.get('msg_type') == 'status' and data.get('content', {}).get('execution_state') == 'idle' and data.get('parent_header', {}).get('msg_id') == 'f1':
            break
    except:
        break
ws.close()
